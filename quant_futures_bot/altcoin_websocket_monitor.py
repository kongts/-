...
class AltcoinWebSocketMonitor:
    def __init__(
        ...
        self.monitor = AltcoinPaperMonitor(
            top=top,
    ...
    def run_forever(self) -> None:
        print(
            f"altcoin websocket monitor started execution_mode=testnet top={self.top} "
        ...
        while True:
            if not self.leaders:
                print(f"[{self.now()}] altcoin_websocket_waiting reason=no leaders", flush=True)
    ...
    def on_open(self, _ws) -> None:
        print(f"[{self.now()}] altcoin_websocket=connected streams={self.stream_labels()}", flush=True)

    def on_close(self, _ws, status_code, message) -> None:
        print(f"[{self.now()}] altcoin_websocket=closed status={status_code} message={message}", flush=True)

    def on_error(self, _ws, error) -> None:
        print(f"[{self.now()}] altcoin_websocket_error={error}", flush=True)
    ...
    def refresh_leaders(self) -> bool:
        ...
        print(
            f"[{self.now()}] altcoin_leaders_changed old={old_labels} new={','.join(self.symbols()) or '-'}",
            flush=True,
        )
        return True
    ...
    def timeframe_seconds(timeframe: str) -> int:
        ...
        raise ValueError(f"Unsupported altcoin websocket timeframe: {timeframe}")
...
def main() -> None:
    ...
    monitor = AltcoinWebSocketMonitor(
        top=args.top,
